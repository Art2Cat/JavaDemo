...
public class RoleSqlProvider {
    public String getRoleByName(String name) {
        SQL sql = new SQL().SELECT("id, role_name as roleName, create_date as createDate, note").FROM("role");
    ...
    public String update(Role role) {
        SQL sql = new SQL().UPDATE("role").SET("role_name=#{roleName}, create_date=#{createDate},note=#{note")
                .WHERE("id=#{id}");
        return sql.toString();
    }
}
