...
public class Stream52 {
    ...
    public void testFind() {
        ...
        Optional<Integer> find = someNumbers.stream().map(x -> x * x)
            ...
            .findFirst();
        if (find.isPresent()) {
            Assertions.assertEquals(Integer.valueOf(9), find.get());
        } else {
            Assertions.fail();
        }
    ...
    public void testReduce() {
        ...
        Assertions.assertEquals(Integer.valueOf(9), max.orElse(1));

        Optional<Integer> min = someNumbers.stream().reduce(Integer::min);
        Assertions.assertEquals(Integer.valueOf(3), min.orElse(1));
    }
}
